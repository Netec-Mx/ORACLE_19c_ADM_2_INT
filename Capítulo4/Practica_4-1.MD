...
```bash
# Verificar que este conectado como usuario oracle
id
...
```sql
-- Conectarse al CDB$ROOT como usuario sys
sqlplus / as sysdba
...
Se valida la existencia del Application Seed (`APP_ROOT$SEED`). Esta PDB especial funciona como **plantilla base** para crear Application PDBs derivadas. En caso de estar cerrada, se abre con `ALTER PLUGGABLE DATABASE app_root$seed OPEN`. 
...
-- Si no existe crearla desde el contenedor APP_ROOT
ALTER SESSION SET CONTAINER=APP_ROOT;
CREATE PLUGGABLE DATABASE AS SEED ADMIN USER admin_seed IDENTIFIED BY oracle_4U;
```
...
-- Si da el error ORA-00959: tablespace 'USERS' does not exist, crearlo.
CREATE TABLESPACE USERS DATAFILE SIZE 100M AUTOEXTEND ON MAXSIZE 2G;

-- Volver a sincronizar con la aplicación
ALTER PLUGGABLE DATABASE APPLICATION hr_app SYNC;
```
...
#### **Paso 4.** Confirmar que las tablas están vacías (solo metadatos) menos config_count.
...
-- Si da el error ORA-00959: tablespace 'USERS' does not exist, crearlo.
CREATE TABLESPACE USERS DATAFILE SIZE 100M 
...
-- Crear el directorio para la PDB app_pdb3
!mkdir /u01/app/oracle/oradata/ORCL/app_pdb3
...
CREATE PLUGGABLE DATABASE app_pdb3
  ...
  ROLES = (DBA)
  CREATE_FILE_DEST = '/u01/app/oracle/oradata/ORCL/app_pdb3';
```
...
BEGIN
    FOR pdb IN (SELECT name, con_id FROM v$containers WHERE application_pdb = 'YES' ORDER BY name) LOOP
        ...
        EXECUTE IMMEDIATE 'SELECT COUNT(*) FROM hr_app_user.departments_app' INTO v_count;
