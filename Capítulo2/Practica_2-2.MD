...
* Cambiar el password del usuario sys y system a: oracle_4U 
...
```sql
-- Cambiar el password de sys y systema a: oracle_4U
sqlplus / as sysdba
alter user sys identified by oracle_4U;
...
```bash
# El listener registra los servicios dinámicamente (espera breve)
sleep 10
...
```
-- Agrega los siguiente servicios al archivo tnsnames.ora usando vi o gedit editor.
...
```bash
-- Recarga los servicios de la base de datos
lsnrctl reload

-- Espera un minuto mientras se publican los servicios y verifica la conectividad a los servicios
tnsping SALES_APP  
tnsping REPORTS_APP  
tnsping SALES_APP_FO
```

```sql
-- Crea un usuario de prueba en PDB1 (si no existe)
sqlplus / as sysdba
ALTER SESSION SET CONTAINER=pdb1;
...
```bash
# Usando la conectividad definida en tnsnames.ora ejecuta el siguiente comando
...
```sql
-- Conectate al contenedor root y crea un nuevo servicio a nivel comun
sqlplus / as sysdba
SHOW CON_NAME;
...
**tnsnames.ora (agregar el servicio)**
...
```bash
-- Pruebe la conectividad al servicio comun
...
-- Conectate en una segunda terminal al usuario SALES_USER 
sqlplus sales_user/oracle_4U@localhost:1521/pdb1

-- Regresa a la terminal original y da la siguiente sentencia
...
```bash
# Logs del listener
cd $ORACLE_BASE/diag/tnslsnr/localhost/listener/trace
...
ALTER SESSION SET CONTAINER=pdb1;
BEGIN 
DBMS_SERVICE.STOP_SERVICE('reports_app'); 
END; 
/
BEGIN 
DBMS_SERVICE.START_SERVICE('reports_app'); 
END; 
/
SELECT name FROM v$services WHERE name='reports_app';
...
ALTER SESSION SET CONTAINER=pdb1;
SELECT name FROM v$services WHERE name IN ('sales_app','reports_app');
```
...
* Trigger de arranque automático de serivios **habilitado** y funcional tras reabrir PDB1.
